...
void send0(char in_dat[],int n)   /// in_dat[]に渡した文字列をn番目まで送信します
{
    for (int i = 0; i<n; i++)
    {
        write(tty_fd0,&in_dat[i],1);
        if (in_dat[i] == '\0') break;
    }
}/****************************************************************** END ***/

/************************************************************* １文字受信 ***/
void receive0(char *out_dat)          /// 受信した1文字をout_datに格納します
{
    char r;
    while(read(tty_fd0,&r,1)<1);
    *out_dat=r;
}/****************************************************************** END ***/
